...
strSql = " select trayid from lpstbl where dn_num ='" & tempDN & "' "
Set rs = Get_OracleRs(strSql)
    ...
    If Not rs.EOF Then
        Do While Not rs.EOF
            ...
            'sOra = "select 料号 from erpdata..tblstocknumsub where 箱号 = '" & trid & "' "
            sOra = "select b.料号 from erpdata..tblPackToHousesub a,  erpdata..tblPackToHouse b where a.入库单编号 = b.入库单编号 and   a.箱号 = '" & trid & "' "
            sNow = Get_SqlStr(sOra)
            If sNow <> PRODUCTID Then
...
'             " AND erpdata.dbo.f_getbigparent(b.TRAYQBOXNUMBER)<>'' "
    ...
    strSql = "select distinct carton from lpstbl where dn_num = '" & tempDN & "'"
    Set rs = Get_OracleRs(strSql)
'    If rs.State = adStateOpen Then rs.Close
'    rs.Open strSql, INIadoCon, adOpenStatic, adLockReadOnly, adCmdText
    CmbQbox.Clear
    If Not rs.EOF Then
        Do While Not rs.EOF
            CmbQbox.AddItem Trim$("" & rs!carton)
            rs.MoveNext
        Loop
        CmbQbox.ListIndex = 0
    End If
