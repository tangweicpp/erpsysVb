...
Dim rs                  As New ADODB.Recordset
    ...
    '加载单据类型
    strSql = "  select distinct a.lot_type as lott from PJ_WO_PRI a where a.lot_type is not null order by  a.lot_type desc"
    If rs.State = adStateOpen Then rs.Close
    rs.Open strSql, Cnn, adOpenStatic, adLockReadOnly, adCmdText
    Combo3.Clear
    If Not rs.EOF Then
        Do While Not rs.EOF
            Combo3.AddItem Trim$("" & rs!lott)
            rs.MoveNext
        Loop
        Combo3.ListIndex = 0
...
Call AddWOPRI(headerTemp.ordername, Trim(Combo1.Text), Trim(Combo3.Text), gUserName)
