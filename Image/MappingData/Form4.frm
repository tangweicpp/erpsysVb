...
strsql = "select ORDERNAME from ib_wohistory where ORDERNAME='" & workName & "'" '先判断工单是否存在
 If Cnn.State = 0 Then
    ConOracle
 End If
 ...
 strsql = "update ib_wohistory set planenddate=to_date('" & dtTemp & "','yyyy-mm-dd') where ORDERNAME='" & workName & "'" '修改工单完工日期主要为MPSWIP报表服务
