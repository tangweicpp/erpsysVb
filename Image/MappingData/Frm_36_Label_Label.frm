...
Begin VB.Form Frm_36_Label 
   ...
   Begin VB.Frame Frame1 
      ...
      Begin VB.Frame Frame2 
         ...
         Top             =   360
         Width           =   4335
         Begin VB.CommandButton CmdCustomRPT 
            Caption         =   "导出ShippingList"
            Height          =   480
            Left            =   2520
            TabIndex        =   15
            Top             =   2040
         ...
         Begin VB.ComboBox CusPT 
            Height          =   315
            ItemData        =   "Frm_36_Label_Label.frx":0000
            Left            =   2040
            List            =   "Frm_36_Label_Label.frx":0002
   ...
   Begin VB.TextBox TxtWaferID 
      ...
      TabIndex        =   1
      Text            =   "Frm_36_Label_Label.frx":0004
      Top             =   600
      Width           =   9255
   End
   ...
   End
End
Attribute VB_Name = "Frm_36_Label"
Attribute VB_GlobalNameSpace = False
Attribute VB_Creatable = False
Attribute VB_PredeclaredId = True
Attribute VB_Exposed = False
Private Sub CmdCustomRPT_Click()
'导出客户报表
...
customerTemp = UCase(Trim(CusPT.Text))

If customerTemp = "" Then
...
'        " from  tsv_qboxnumber_WaiBao_36 a , ib_waferlist b  ,   ib_wohistory c, mappingdatatest e, container d, product p , productbase pb " & _
'       " Where b.waferid = replace(a.waferscribenumber,'-A','') And c.OrderName = b.OrderName And p.productbaseid = pb.productbaseid " & _
'        " and pb.productname=c.product and d.containername=a.waferscribenumber and  e.customershortname='" + customerTemp + "' and e.substrateid=replace(a.waferscribenumber,'-A','') "
'
'
 
 sql1 = " select distinct 'HTKS' as Supplier, 'MG'as Customer,  p.alternatename as Device, cus.po_num as PONO, b.waferlot as LOTID, replace(a.waferscribenumber, '-A', '') as WaferID, " & _
        " a.qty as GoodDiePcs, Get_MGVTQty(replace(a.waferscribenumber, '-A', ''),'-E') as HTNGDIEPcs, Get_MGVTQty(replace(a.waferscribenumber, '-A', ''),'-F') as CustomerNGDiePcs, " & _
        " to_char(sysdate, 'YYYY-MM-DD') as ShippingDate,  '' as 备注 " & _
        " from tsv_qboxnumber_WaiBao_36 a,ib_waferlist b, ib_wohistory c, mappingdatatest e, container d, product p, productbase pb,customeroitbl_test cus " & _
        " Where b.waferid = replace(a.waferscribenumber, '-A', '')  And c.OrderName = b.OrderName And p.productbaseid = pb.productbaseid " & _
        " and pb.productname = c.product and d.containername = a.waferscribenumber and e.customershortname = 'MG' " & _
        " and e.substrateid = replace(a.waferscribenumber, '-A', '') and cus.source_batch_id=e.lotid  "

sql3 = "  order by  b.waferlot,    replace(a.waferscribenumber, '-A', '') "
...
For i = 0 To UBound(bid) - 1
    ...
    cmdStr2 = " insert into tsv_qboxnumber_WaiBao_36 select containerid,containername,qty,sysdate from container where containername= '" & lotStr & "-A' "
...
sqlDB = GetWaiBao36String(txtStrTemp3)
...
TxtWaferID.Text = ""
TxtDir.Text = "\\10.160.1.14\BarCode\36\36外挂\"
