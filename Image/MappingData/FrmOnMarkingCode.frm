...
'invTemp = UCase(Trim(TxtInvoice.Text))
'shipTemp = UCase(Trim(TxtShiping.Text))
...
'判断这笔发货单号，是否正确
If Trim(TxtOpn.Text) = "" Then     'Or Trim(TxtInvoice.Text) = "" Or Trim(TxtShiping.Text) = ""
...
cmdStr = " insert into CUSTOMERMarkingCode (id,mpnpart,wsgpart,flag,Qtech_Created_By,Qtech_Created_Date,markcodefirst,ZX_REQUESTNUMBER,ZX_SHIPPER,ZX_SELFPICKUP,ZX_TRACKINGNO,ZX_SPECIALMARK) values " & _
" (" & idTemp & ",'" & billNoTemp & "','" & invTemp & "','Y','" & username & "',sysdate,'" & shipTemp & "','" & strTmp(0) & "','" & strTmp(1) & "','" & strTmp(2) & "','" & strTmp(3) & "','" & strTmp(4) & "') "


cmdStr2 = " insert into [erpdata].[dbo].[TSVtblMPSInvoice] (id,单据编号,InvoiceNo,CreateDate,shipno,ZX_REQUESTNUMBER,ZX_SHIPPER,ZX_SELFPICKUP,ZX_TRACKINGNO,ZX_SPECIALMARK) values " & _
" (" & idTemp & ",'" & billNoTemp & "','" & invTemp & "',getdate(),'" & shipTemp & "','" & strTmp(0) & "','" & strTmp(1) & "','" & strTmp(2) & "','" & strTmp(3) & "','" & strTmp(4) & "') "
