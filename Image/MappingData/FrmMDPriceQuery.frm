...
sqlTemp = " select   ID ,CUSTOMERSHORTNAME,CUSTOMERNAME ,PO_NUM ,PO_DATE ,qtech_created_date, PO_TYPE ,PT , PeaceQty,'' as shengyuqty, QTY ,PRICE ,UNIT , " & _
"  FILENAME   from  TSV_MD_POPrice where flag='Y'  and  PO_NUM ='" & billNoTemp & "'order by id desc "
...
Else
    ...
    'productTemp = UCase(Trim(CmbType.Text))
     ...
     sqlTemp = " select   ID ,CUSTOMERSHORTNAME,CUSTOMERNAME ,PO_NUM ,PO_DATE ,qtech_created_date, PO_TYPE ,PT ,PeaceQty, '' as shengyuqty,QTY ,PRICE ,UNIT , " & _
     "  FILENAME   from  TSV_MD_POPrice where flag='Y'  and  PO_DATE >=to_date('" + date1Temp + "','YYYY-MM-DD')  and  PO_DATE <to_date('" + date2Temp + "','YYYY-MM-DD')  "
