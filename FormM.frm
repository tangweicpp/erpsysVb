...
If CmbCustomer.Text = "" Or TxtSourceBatchId.Text = "" Then
    MsgBox "客户信息，SourceBatchId 不可以为空!", vbInformation, "友情提示"
    Exit Sub
Else
...
'获取文件名
    If InStrRev(Trim(TxtExcelDir.Text), "\") > 0 Then
        strfilename = Mid(Trim(TxtExcelDir.Text), InStrRev(Trim(TxtExcelDir.Text), "\") + 1)
        dirName = Mid$(Trim(TxtExcelDir.Text), 1, InStrRev(Trim(TxtExcelDir.Text), "\"))
    End If
...
con.open "Provider=Microsoft.Jet.OLEDB.4.0;Persist Security Info=False;Data Source=" & TxtExcelDir.Text & ";Extended Properties='Excel 8.0;HDR=Yes'"

Rs.open "Select * From [sheet1$]", con, adOpenStatic, adLockReadOnly, adCmdText
...
For i = 0 To Rs.RecordCount - 1
    ...
    For j = 0 To Rs.fields.Count - 1
        temp = temp & "," & newStr("" & Rs.fields(j).Value)
    ...
    id = GetUploadMaxID()
    temp = id & temp
    temp = temp & ",'Auto',sysdate,'Y' "

    Call AddUploadDetail(temp)
...
Set listRS = GetFpsUploadTempData()
...
Private Sub Command3_Click()
 ...
 sqlTemp = "select SEQ_IBWO,ORDERNAME,ORDERTYPE,DESCRIPTION,EVENTTYPE,ERPUSER,PRODUCT,PRODUCTREVISION,QTY,PRODUCTBOM,ERPCREATEDATE,PLANSTARTDATE,PLANENDDATE," & _
         ...
         " From IB_WOHISTORY_TEST order by SEQ_IBWO "
...
Private Sub Command4_Click()
 ...
 Dim sqlTemp As String
 sqlTemp = "select ORDERNAME,WAFERID,COMPLETEFLAG,DIEQTY,FGDIEQTY,WAFERLOT,WAFERSEQUENCE,MARKINGCODE from IB_WAFERLIST_TEST order by ORDERNAME, WAFERID"
  ExporToExcel (sqlTemp)
...
'附值
 ...
 headerTemp.OrderName = Text2.Text
...
TxtRequestDate.Value = TxtDate.Text
